...
local yearlist "1849 1858 1866 1883 1882 1888 1892 1896 1901"
...
foreach year of local yearlist {
	...
	*USE LIST OF COUNTIES
	...
	keep kreiskey`year' kreiskey1800

	duplicates drop
	
	drop if kreiskey1800 == .
	
	*NEED TO CHECK THIS IS OK LATER - HOW MANY OBS ARE WE DROPPING BY DOING THIS?
	duplicates tag kreiskey`year', gen(dup)
	drop if dup
	drop dup
	...
	*COUNTIES, RATHER THAN TAKING THE AVERAGE
	collapse (sum) `vars', by(kreiskey1800)
...
*LIST OF YEARS TO LOOP THROUGH
local yearlist "1858 1866 1882 1883 1886 1888 1892 1896 1901"
...
foreach year of local yearlist {
	...
	*MERGE BASED ON THE HIGH LEVEL KREISKEY
	merge 1:1 kreiskey1800 using "$data/year_merged_summed/`year'"
	drop _m
